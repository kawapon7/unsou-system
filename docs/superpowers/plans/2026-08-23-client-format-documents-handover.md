...
**既知の制限**: 走行距離は列が無く空欄／作業系判定は案件名ベース（次版で `projects` にカテゴリ列）／人員結果表（別紙）は次版／相殺額ラベルは適格事業者でも「0%分」と出る／明細シート合計は行の合計、本票はDB値のため `work_records` を後から直すとズレうる／Excel 利益表の「売上（１０％分）」は 売上（税抜）×10%＝おおば運送の標準マージン（ボス確認 2026-08-23）。案件により率が前後するが v1 は一律10%。売上・支払の両方を持っているので将来は実マージン（売上−支払）に切替可能。
