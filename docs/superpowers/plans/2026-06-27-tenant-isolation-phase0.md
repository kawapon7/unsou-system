# テナント分離 フェーズ0（下ごしらえ）実装計画（❌ 廃止・参照禁止）

> **⚠️ この計画は 2026-07-27 に廃止された。`2026-07-27-tenant-isolation-phase0.md` を使うこと。**
>
> 廃止理由:
> 1. `companies` をテナントマスタ兼用にする前提だったが、2026-07-26の自社マスタ実装で
>    `companies` は「請求書の発行元情報」の表として `tenant_id text UNIQUE` を持つ形になり、
>    本計画のTask 1（`companies.id` をテナントUUIDにする）と正面衝突する。
> 2. `driver_project_assignments` を「uuid型・変換不要」と記載しているが誤り。
>    2026-07-26のバグ修正で text に統一済みのため、変換対象に含める必要がある。
> 3. 不変ログ（`approval_history` / `notification_logs`）の backfill にトリガー一時無効化＋UPDATEを
>    使う手順になっており、CLAUDE.md §2 の不変ログ規約に抵触する。
>
> 以下は履歴として残す。実装には使わないこと。
