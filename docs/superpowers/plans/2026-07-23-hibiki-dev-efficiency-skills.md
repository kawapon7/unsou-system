...
**Architecture:** `.claude/skills/` 配下に2つのプロジェクトスキルを新規作成。hibiki-dev はタスク冒頭で自動発動し「労力のライトサイジング」と「仕様ナビ」で入口を整理する交通整理役。hibiki-security は git操作やDB/口座/不変ログに触れる時だけ呼ばれる軽量ゲートで、規約本文は複製せず既存（CLAUDE.md / HANDOVER §2-6S）を指すのみ。最後に CLAUDE.md のスキル強制起動ルールを「最適選択」へ緩和する。
...
**Interfaces:**
- Consumes: なし（新規）
- Produces: スキル `hibiki-dev`。開発タスク冒頭で自動発動し、ライトサイジング宣言＋仕様ナビ手順を提供。hibiki-security を「口座/DB/不変ログ/git」検知時に呼ぶ参照を含む。
...
---
name: hibiki-dev
description: HIBIKI（運送業SaaS）の実装・調査・修正タスクを始めるときに使う。労力のライトサイジングと仕様ナビで、時間とトークンを節約しつつ着手する入口スキル。手動起動は /hibiki-dev。
---
...
Run:
```bash
grep -c -E "セキュリティ先行チェック|ライトサイジング|仕様ナビ|grep -nE|全文読み禁止|hibiki-security" .claude/skills/hibiki-dev/SKILL.md
```
Expected: `6`（6要素すべて存在）。
...
git status
git commit -m "feat: hibiki-dev スキル追加（ライトサイジング＋仕様ナビで開発を効率化）"
```
...
hibiki-dev が発動し（ライトサイジング宣言＋§5-6 のピンポイント読み）、
...
**1. Spec coverage:**
- 2スキル体制 → Task 1, 2 ✓
- ライトサイジング（4段階＋機械的先行チェック＋安全弁） → Task 1 手順0/1 ✓
