...
## ⏱ 着手前に必ず: URL変更を先に済ませること

**本番URLの `kawapon7` → `hibiki` への変更を、この作業より先に完了させる**（2026-08-17 決定）。

新プロジェクトには Redirect URLs の設定が必要なため、URLが未確定のまま作ると
「旧URLで設定 → あとで変更 → 新DB側をやり直し」で2回になる。URLを先に確定すれば1回で済む。
