...
## ⏱ 着手前に必ず: URL変更を先に済ませること → ✅ 2026-08-17 完了

**本番URLは `https://unsou-system.hibiki-app.workers.dev` で確定済み**
（`hibiki` は取得済みで使えず `hibiki-app` を採用）。

⚠️ **新プロジェクトの Site URL / Redirect URLs はこの新URLで設定すること。**
Redirect URLs は `https://unsou-system.hibiki-app.workers.dev/**` の形。

（先にURLを確定させた理由: 未確定のまま新DBを作ると
「旧URLで設定 → あとで変更 → 新DB側をやり直し」で2回になるため。）

⚠️ **この作業はまとまった時間が取れるときに始めること。** 手数が多く、途中で中断すると
「画面は旧DB・サーバーは新DB」という危険な中間状態が生まれる。URL変更とは別セッションで。
