...
## 12. ベースライン記録（2026-07-29 実施）

### 12-1. 実施方法の変更

当初は 4 経路を UI から実行して記録する計画だったが、**ブラウザ自動操作がこのアプリで機能しなかった**ため、
**スキーマとコードの突き合わせによる論理的確定**に切り替えた。結果として、UI 実行より強い証拠が得られている
（「たまたま動いた／動かなかった」ではなく「必ずこうなる」が言えるため）。

**UI 自動操作の問題（未解決・後続タスクに影響する）:**
`computer` ツールの `left_click` / `type` がページに届かない。ボタンは `disabled=false`、
`document.elementFromPoint()` でも最前面にボタン本体が返り、DOM 上の重なりは無い。それでも
サーバアクションが一切発火しない。`form_input` は機能する（プレビュー計算が走った）ため、
合成イベントの一部だけが通っていない状態。**Task 4 以降の画面検証は別の手段が必要**（§12-4 参照）。

### 12-2. 実測したDBの状態（2026-07-29 時点）

| 項目 | 値 |
|---|---|
| `invoices` の全行数 | **1 件** |
| その内容 | 【デモ】スポット配送サービス合同会社 / `2026-07-01` / `issued` / ¥147,950 / 作成 2026-07-29 07:09 UTC |

この 1 件は 2026-07-29 に `upsertInvoice` を実地確認したときのもの。**それ以外の経路は本番で一度も成功していない。**

### 12-3. 経路ごとの判定

`invoices` の「NOT NULL かつ DEFAULT なし」の列は **4 つだけ**（実測）:
`client_id` / `target_month` / `total_amount_ex_tax` / `total_tax`

一意制約は **`UNIQUE (client_id, invoice_month)` が存在する**（実測。2026-07-28 追加分）。

| # | 経路 | 判定 | 根拠 |
|---|---|---|---|
| 1 | `finalizeInvoice` | **動作するはず**（未実測） | 必須 4 列をすべて渡している。`onConflict: 'client_id,invoice_month'` に対応する制約も実在する |
| 2 | `saveClientScanResult` | **1 枚目は成功・2 枚目は必ず失敗**（B-2 確定） | 必須 4 列は渡している。ただし素の `insert` のため、同一荷主・同一月の 2 枚目が `UNIQUE(client_id, invoice_month)` に当たり `23505` |
| 3 | `upsertInvoice` | **成功**（実測済み 2026-07-29） | DB に 1 件残っている |
| 4 | `commitManualInvoice` | **必ず失敗**（B-1 確定） | 渡しているのは `client_id` / `invoice_month` / `status` / `total_amount` / `tenant_id` / `updated_at` のみ。**必須列 `target_month`・`total_amount_ex_tax`・`total_tax` の 3 つすべてが欠落** → `23502` |

**B-3 について補足:** `commitManualInvoice` は素の `insert` でもあるため 2 回目の `23505` も抱えているが、
**B-1 の `23502` が先に発生するため表面化しない。** B-1 を直すと B-3 が露出する。共通ライタは両方同時に解消する。

### 12-4. 後続タスクへの申し送り

1. **UI 自動操作が効かない。** Task 4 以降の画面検証は、①ボスが手で操作して結果を伝える、
   ②`preview_logs` のサーバアクション呼び出しログで間接的に確認する、③検証を本番デプロイ後に
   ボスが実施する、のいずれかに切り替える必要がある。**「画面で確認した」と書けない場合は必ず「未検証」と明記すること。**
2. **AI スキャンの画面パスは `/admin/scan`。** 計画書に書いた `/admin/sales?tab=scan` は誤り。
3. **`commitManualInvoice` の `finalAmount` は税込。** 画面のプレビューで確認した
   （税抜 ¥10,000 → 消費税 +¥1,000 → 経過措置 −¥220 → 最終請求額 ¥10,780）。
   計画書 Task 4 Step 3 の「税込か税抜か要確認」はこれで解決。**税抜額と税額を別途渡すよう実装すること**
   （`finalAmount` をそのまま `subtotal` に入れると旧列 `total_amount_ex_tax` に税込額が入ってしまう）。
