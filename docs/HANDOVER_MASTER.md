...
| 🟡 中 | 請求書生成のUI経由での実地確認（未実施） | 7/28の不具合5件修正（`7f5ba66`）は**DB上のロールバックテストで検証したのみで、UI経由では未確認**。本番 `invoices` は依然 **0件** ＝ 画面から一度も請求書が作られていない。自動デプロイはCI success（2026-07-28 07:14:50Z）。**残作業＝`/admin/billing` で請求書を生成し、①エラーが出ないこと ②`invoices` に行が増えること ③請求書PDFが出て金額が承認額と一致すること（`invoice_month` 検索への統一が効いているか）を確認する。** 参考: 現在のデータは clients 6 / work_records 125 / expense_records 19 / payment_notices 13 |
