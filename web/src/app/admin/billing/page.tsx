...
function calcPaymentDate(yearMonth: string, paymentSite: number): string {
  const [y, m] = yearMonth.split('-').map(Number)
  const monthEnd = new Date(y, m, 0)
  monthEnd.setDate(monthEnd.getDate() + paymentSite)
  return `${monthEnd.getFullYear()}年${monthEnd.getMonth() + 1}月${monthEnd.getDate()}日`
}
...
export default function BillingPage() {
  ...
  // アラートパネルからの遷移時に対象月を自動セット
  useEffect(() => {
    const m = searchParams.get('month')
    if (m && /^\d{4}-\d{2}$/.test(m)) setYearMonth(m)
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              ...
              対象稼働期間：
              {yearMonth.replace(/^(\d{4})-(\d{2})$/, (_, y, m) => {
                const lastDay = new Date(Number(y), Number(m), 0).getDate()
                return `${y}年${Number(m)}月1日〜${lastDay}日`
              })}
            </p>
          </div>
        </div>
