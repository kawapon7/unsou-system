 ...
 * DETAIL 行除去 → トークン → メール → 数字列（数字列を先にやるとメール/トークン判定が崩れる）。
 */
export const MESSAGE_MAX = 2000
export const STACK_MAX   = 4000

// Postgres の DETAIL は行データ（Failing row contains / Key (col)=(値)）を露出する。
// 括弧の入れ子や書式に依存せず、DETAIL: 以降を行末まで丸ごと落とす。
...
export function mask(text: string, maxLen: number): string {
  let out = text
    .replace(PG_DETAIL, 'DETAIL: [omitted]')
    .replace(JWT, '[token]')
